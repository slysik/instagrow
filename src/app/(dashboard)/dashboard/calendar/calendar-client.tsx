...
export function CalendarClient({
	...
	const handleMoveEvent = async (eventId: string, newDate: Date) => {
		try {
			...
			const endDate = new Date(viewMode === "weekly" ? weekStart : monthStart);
			if (viewMode === "weekly") {
					...
					{isEmpty && (
						<div className="card text-center py-12">
							...
							<p className="text-gray-600 mb-6 max-w-md mx-auto">
								Start planning your content by scheduling ideas from your
								content library. Go to the content library to create and
								schedule posts.
							</p>
							<a
										...
										<div className="space-y-2">
											{dayEvents.map((event) => {
												const config =
													postTypeConfig[
														event.idea
															.contentType as keyof typeof postTypeConfig
													];
							...
							{feedMixRules.map((rule) => {
								...
								return (
									...
									>
										<div className="flex items-center gap-2">
											<div
												className={`w-8 h-8 ${rule.color} rounded-lg flex items-center justify-center`}
											>
